...
| `brain-map-visualization` | Load when task involves brain map, cognition visualization, session journal graph, or visualizing handoff/note co-access. Runs parser and guides user to viewer. |
| `browser-review-protocol` | Structured manual or MCP-driven frontend review—URL, auth, viewports, flows, evidence. Use for smoke UI, PR UI check, repeatable browser QA. |
| `docs` | Use when writing or updating documentation, README, API docs, runbooks, or user-facing text. Use for structure, clarity, links, and keeping docs up to date with code. Load when the user asks to document, write README, or update docs. |
| `gsd-workflow` | When to use get-shit-done (GSD) spec-driven commands vs OpenHarness planning skill and native verification; pointers only — does not install upstream for you. |
| `learn-claude-code-harness` | Maps shareAI-lab learn-claude-code teaching sessions to OpenHarness skills; thin reference — no fork of upstream. |
